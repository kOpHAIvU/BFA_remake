...
class BFA(object):
    ...
    def progressive_bit_search(self, model, data, target):
        ...
        for m in model.modules():
            if isinstance(m, quan_Conv2d) or isinstance(m, quan_Linear) or m.__class__.__name__ == "CustomBlock" or m.__class__.__name__ == "quan_Conv1d":
        ...
        while self.loss_max <= self.loss.item():
            ...
            # Check if self.loss_dict is not empty
            if self.loss_dict:
                # Find the layer with the max loss
                max_loss_module = max(self.loss_dict.items(), key=operator.itemgetter(1))[0]
                self.loss_max = self.loss_dict[max_loss_module]
            else:
                print("self.loss_dict is empty, stopping the search")
                break  # Exit the loop if loss_dict is empty
    ...
    def random_flip_one_bit(self, model):
        ...
        for name, m in model.named_modules():
            if m.__class__.__name__ == "CustomBlock":
